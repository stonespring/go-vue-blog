...
let portUrl = "http://go.yizheshuo.top/api/";
...
const ArtClassData = (callback) => {
    if(sessionStorage.getItem('classList')){
        ...
        let url = portUrl + 'column_list';
        axios.get(url).then(num => {
            // console.log(num);
            if(num.data.code==200){
                sessionStorage.setItem('classList',JSON.stringify(num.data.data.data));
                callback && callback(num.data.data.data)
            }else{
                alert("查询失败")
            }
        })
